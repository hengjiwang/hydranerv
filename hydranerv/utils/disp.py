...
def add_spike_trains(ax, spk_trains, fps, lw=None, color='#1f77b4', inc=0):
    """Add a scatter plot of spk_trains to ax"""

    if type(fps) == int:
        for j in range(len(spk_trains)):
            ax.vlines(np.array(spk_trains[j])/fps, inc+j, inc+j+1, lw=lw, color=color)

    elif type(fps) == list:
        for j in range(len(spk_trains)):
            ax.vlines(np.array(spk_trains[j])/fps[j], inc+j, inc+j+1, lw=lw, color=color)
