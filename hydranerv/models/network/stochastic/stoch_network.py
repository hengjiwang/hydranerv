...
from hydranerv.models.network.stochastic.stoch_neuron import StochNeuron
    ...
    def setup(self):
        ...
        for i in range(self.num):
            if i in self.pacemakers:
                nrn = StochNeuron(self.dt, self.tmax, np.random.uniform(-self.wnoise, self.wnoise), True, self.t_ref)
                if self.is_semi_pm:
                    nrn.k_in = 2 * nrn.k_in
                ...
                self.neurons.append(StochNeuron(self.dt, self.tmax, np.random.uniform(-self.wnoise, self.wnoise), False, self.t_ref))
...
if __name__ == '__main__':
    ntwk = StochNetwork(num=10, gc=500, pacemakers=[0], conn_type='gap_junction')
