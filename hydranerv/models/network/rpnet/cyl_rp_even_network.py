...
class CylRPEvenNetwork(RPNetwork):
    """a model of cylindrical neuronal networks where neurons are evenly distributed"""
    def __init__(self, num_lon=15, num_cir=15, gc=1, dt=.01, tmax=1000,
                t_ref=.1, conn_type='gap_junction', t_syn=.01,
                prob_lon=1, prob_cir=1, rho=.5, phi_offset=np.pi/36, seed=0):
        """constructor"""
        ...
        super().__init__(num_lon * num_cir, [], gc, dt, tmax, t_ref, conn_type,
                        t_syn, seed)
    ...
    def set_locations(self):
        ...
        for i, z in enumerate(np.arange(1, -dz/2, -dz)):
            for j, phi in enumerate(np.arange(self.phi_offset, 2 * np.pi + self.phi_offset, dphi)):
